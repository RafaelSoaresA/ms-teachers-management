...
class InstructorToTeacherOutputMapperTest {
    ...
    void responseGetTeacher_withNullInstructor_shouldReturnEmptyTeacher() {
        TeacherDtoOutput result = mapper.map(null, TeacherDtoOutput.class);
    ...
    @DisplayName("Deve lançar NullPointerException quando wage for nulo")
    @Test
    void responseGetTeacher_withNullWage_shouldThrowNullPointerException() {
        Instructor instructor = new Instructor();
        instructor.setId(1);
        instructor.setFullName(null);
        instructor.setWage(null);
        Course course = new Course();
        course.setTitle(null);
        instructor.setCourse(course);

        assertThrows(NullPointerException.class, () -> mapper.map(instructor, TeacherDtoOutput.class));
    }

    @DisplayName("Deve lançar NullPointerException quando course for nulo")
    @Test
    void responseGetTeacher_withNullCourse_shouldThrowNullPointerException() {
        Instructor instructor = new Instructor();
        instructor.setId(1);
        instructor.setFullName(null);
        Wage wage = new Wage();
        wage.setTotal(null);
        wage.setCurrency(null);
        instructor.setWage(wage);
